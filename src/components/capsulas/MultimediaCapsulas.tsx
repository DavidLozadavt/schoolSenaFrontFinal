...
import { Film, BookImage, Play, ChevronLeft, ChevronRight, X } from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 bg-black/95 z-[9999] flex items-center justify-center animate-fade-in" onClick={onClose}>
      ...
      >
        ...
        {/* Media with unique key to prevent reconciliation errors */}
        ...
        <div className="absolute bottom-0 left-0 right-0 p-8 z-30 pointer-events-none">
          <div className="flex items-center gap-2 mb-2">
             <span className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold text-white ${currentGroup.tipo_item === 'reel' ? 'bg-purple-600' : 'bg-blue-600'}`}>
                {currentGroup.tipo_item === 'reel' ? <Film className="w-3 h-3" /> : <BookImage className="w-3 h-3" />}
                {currentGroup.tipo_item.toUpperCase()}
             </span>
          </div>
          <h3 className="text-white font-black text-xl mb-1">{currentGroup.nombreGrupo}</h3>
          <p className="text-white/60 text-sm font-medium">Cápsulas formativas SENA</p>
        </div>
...
const MultimediaCapsulas = () => {
  ...
  const getThumbnail = (item: MultimediaItem) => {
    ...
    // Si es YouTube, devolver miniatura de YouTube
    const ytMatch = url.match(/(?:youtube\.com\/(?:[^\/]+\/.+\/|(?:v|e(?:mbed)?)\/|.*[?&]v=)|youtu\.be\/)([^"&?\/\s]{11})/i);
    if (ytMatch) {
      return `https://img.youtube.com/vi/${ytMatch[1]}/hqdefault.jpg`;
    }

    // Si es una URL externa (TikTok, Instagram, etc) y no es un archivo de video directo, 
    // no devolvemos la URL como miniatura para evitar errores de carga en <img>
    const isExternal = url.includes('tiktok.com') || url.includes('instagram.com') || url.includes('facebook.com');
    if (isExternal && !isVideo(url)) {
      return ''; // No hay miniatura de imagen directa
    }
          ...
          return (
            ...
            >
              ...
              {thumb ? (
                ...
                <div className={`absolute inset-0 bg-gradient-to-br ${isReel ? 'from-purple-900 to-indigo-900' : 'from-blue-900 to-cyan-900'} flex items-center justify-center`}>
                   <div className="flex flex-col items-center gap-2 opacity-40 group-hover:opacity-60 transition-opacity">
                      ...
                      <span className="text-[10px] font-black text-white uppercase tracking-widest">{isReel ? 'Reel' : 'Story'}</span>
                   </div>
                </div>
              )}
              ...
              <div className="absolute bottom-0 left-0 right-0 p-3 z-10">
                <h4 className="text-white font-bold text-[11px] sm:text-xs leading-tight mb-1 line-clamp-2">
                  {item.nombreGrupo}
                </h4>
