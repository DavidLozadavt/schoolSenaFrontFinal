...
interface VerDocumentosFichaModalProps {
  isOpen: boolean;
  onClose: () => void;
  ficha: Ficha | null;
}

export const VerDocumentosFichaModal = ({
  isOpen,
  onClose,
  ficha,
}: VerDocumentosFichaModalProps) => {
  ...
  useEffect(() => {
    if (isOpen && ficha?.id) {
      setLoading(true);
      axios
        .get(`ficha/${ficha.id}/documentos-pide`)
        .then((res) => {
  ...
  const label = ficha?.grado?.nombreGrado || `Ficha #${ficha?.id}`;
        ...
        <div className="p-5 max-h-80 overflow-y-auto">
          {loading ? (
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 italic py-4 text-center">
              No hay documentos asignados a esta ficha.
            </p>
          ) : (
