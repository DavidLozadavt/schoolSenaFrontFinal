...
const FormularioUpCentrosFormacion: React.FC<Props> = ({
  idCentroFormacion,
  ...
  const formik = useFormik<FormValues>({
    ...
    onSubmit: async (values, { setSubmitting }) => {
      try {
        ...
        await axios.patch(`centrosFormacion/${idCentroFormacion}`, payload);
        alert('Centro de formación se actualizó correctamente');
        setEvento((prev) => !prev);
      } catch (error: any) {
  ...
  useEffect(() => {
    ...
    const loadRegional = async () => {
      try {
        ...
        alert('Error al cargar el centro de formación');
      }
    };
...
export default FormularioUpCentrosFormacion;
