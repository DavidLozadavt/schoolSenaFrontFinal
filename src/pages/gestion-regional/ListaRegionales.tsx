...
const ListaRegionales: React.FC<Props> = ({ searchTerm, evento, setEvento }) => {
  ...
  const filteredRegionales = useMemo(() => {
    ...
    return regionales.filter(
      (regional) =>
        regional.razonSocial.toLowerCase().includes(term) ||
        regional.nit.toLowerCase().includes(term) ||
        regional.representanteLegal.toLowerCase().includes(term) ||
        regional.direccion.toLowerCase().includes(term) ||
        regional.email.toLowerCase().includes(term) ||
        regional.ciudad?.descripcion.toLowerCase().includes(term)
    );
...

      {
        ...
        cell: ({ row }) => (
          <button
            ...
            onClick={() => {
              setIdRegional(String(row.original.id));
              setIsModalOpen(true);
            }}
          >
