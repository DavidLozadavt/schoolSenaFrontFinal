...
export const EventForm = () => {
  ...
  const fetchActividades = async () => {
    try {
      if (id) {
        // Fetch event activities and available ones
        const resCon = await axios.get('/items', { params: { idEvento: id } });
        const conEventoList = resCon.data || [];
        
        const resSin = await axios.get('/items', { params: { sinEvento: true } });
        const sinEventoList = resSin.data || [];
        
        const mergedList = [
          ...conEventoList, 
          ...sinEventoList.filter((s: any) => !conEventoList.some((c: any) => c.id === s.id))
        ];
        ...
        setAvailableActividades(resSin.data || []);
      }
    } catch (err) {
            ...
            <div className="p-8 rounded-[2.5rem] bg-neutral-50 dark:bg-white/[0.02] border border-neutral-200 dark:border-white/5 space-y-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-bold text-neutral-400 uppercase tracking-widest">Asocia actividades existentes o crea nuevas para este evento</p>
                  </div>
                </div>
              ...
              {availableActividades.length > 0 && (
                <div className="relative">
                  ...
                  <input
                    type="text"
                    placeholder="Buscar actividades disponibles por nombre o descripción..."
                    value={activitySearchQuery}
                    onChange={(e) => setActivitySearchQuery(e.target.value)}
                    className="w-full bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-white/10 rounded-2xl h-11 pl-12 pr-4 font-semibold text-sm text-neutral-800 dark:text-white transition-all outline-none"
                  />
                </div>
              )}
